/* 
 * 40 procs, 1 threads each .... 3.97
 * 20 procs, 2 threads each .... 3.68 
 * 10 procs, 4 threads each .... 3.76
 * 2 procs, 20 threads each .... 3.79
 * 1 procs, 40 threads each .... 3.85
 */
...
int main(int argc, char *argv[]) {
  ...
  //check if threads are correct ]
  int thread_correct = 1; 
#pragma omp parallel num_threads(NT)
  {
    if(omp_get_num_threads() < NT){
    printf("Not enough threads for proc %d\n", myRank);
    thread_correct = 0; 
    }  
  }

  if(!thread_correct) return 1; 
  ...
  if(myRank==0){
    printf("%d procs, %d threads per proc ..... %.2f\n", nprocs, NT, time);
  }
  MPI_Finalize();
}
