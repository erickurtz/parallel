...
double *dev_u_prev, *dev_u_curr, *dev_u_next;
...
__global__ void kernel(double *prev, double *curr, double* next, int nx, double k){
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  
  if ( tid > 0 && tid < nx-1){ 
    next[tid] = 2.0*curr[tid] - prev[tid] + k*(curr[tid+1] + curr[tid-1] - 2.0*curr[tid]); 
  }  
...
void callKernel(){
  ...
  cudaMemcpy(dev_u_prev, u_prev, nx*sizeof(double), cudaMemcpyHostToDevice);
  cudaMemcpy(dev_u_curr, u_curr, nx*sizeof(double), cudaMemcpyHostToDevice);
  cudaMemcpy(dev_u_next, u_next, nx*sizeof(double), cudaMemcpyHostToDevice);

  
  kernel<<<blocksPerGrid,threadsPerBlock>>>( dev_u_prev, dev_u_curr, dev_u_next,nx,k);  
  
  cudaMemcpy(dev_u_next, u_curr, nx*sizeof(double), cudaMemcpyDeviceToHost);
  double *tmp; 
...
int main(int argc, char *argv[]) {
  ...
  assert(argc==3);
  init(argv[1], argv[2]);
  blocksPerGrid = imin(32, (nx+threadsPerBlock-1)/threadsPerBlock);
  
  cudaMalloc( (void**) &dev_u_prev, nx*sizeof(double));
  cudaMalloc( (void**) &dev_u_curr, nx*sizeof(double));
  cudaMalloc( (void**) &dev_u_next, nx*sizeof(double));
